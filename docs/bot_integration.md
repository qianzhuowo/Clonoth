...
`attachments` 为可选字段。当用户发送图片时，Adapter 需要先将图片保存到 `data/attachments/` 目录下，再将路径填入 `attachments` 数组。即使 `text` 为空，只要 `attachments` 非空，消息也会被接受。
...
| 类型 | 说明 |
|---|---|
| `outbound_message` | 最终回复。把 `payload.text` 发给用户。如果 `payload.attachments` 存在，还需发送图片。 |
...
用户发送图片时，Adapter 需要先下载图片到本地，保存到 `data/attachments/{conversation_key}/` 目录下，再将相对路径填入 inbound 请求的 `attachments` 字段。收到 outbound 中的图片附件时，读取对应文件发送给用户。
