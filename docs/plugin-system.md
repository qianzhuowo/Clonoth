...
内置 handler 在每次进入 AI 节点时通过 `register_builtins()` 注册。注册是幂等的，因为 `HookRegistry` 会按 `handler.name` 替换旧实例。
...
新增 hook handler 时，先确定要插入的 hook point，再判断是否需要外部插件形式。如果逻辑属于引擎核心行为，可放在 `engine/hooks/builtin/` 并加入 `register_builtins()`；如果逻辑属于本地定制，应放入 `plugins/`。
